...
import { User, School, CheckSquare, MessageCircle } from "lucide-react";
            ...
            .map((user) => (
              <tr key={user.id} className="text-center">
                ...
                <td className="px-4 py-2 border">
                  {user.status === "true" ? "Baneado" : "Desbaneado"}
                </td>
...
const ApproveInstitutions = () => {
  const institutions = InstitutionsData((state) => state.institutions);
  const updateInstitutionStatus = InstitutionsData(
    (state) => state.updateInstitutionStatus
  );
  const [loading, setLoading] = useState(false);
            ...
            <tr key={institution.id} className="text-center">
              ...
              <td className="py-2 border flex justify-center gap-3">
                <button
                  ...
                  className={`px-2 py-1 bg-green-500 hover:bg-green-600 transition-all duration-300 text-white rounded ${
                ...
                <button
                  ...
                  className={`px-2 py-1 bg-red-400 hover:bg-red-500 transition-all duration-300 text-white rounded ${
                    loading ? "opacity-50 cursor-not-allowed" : ""
                  } flex flex-row-reverse gap-1 items-center justify-center`}
                  disabled={loading}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
